    ...
    if (message.isStreamer || message.isMod || message.senderUsername === process.env.DEV_NICK) {
        if (client.channelConfigs[message.channelName].isPaused) {
            ...
            client.log.logAndReply(
                message,
                `Eu já estou pausado. Se quiser me despausar, use ${message.commandPrefix}unpause ${emote}`
            );
            return;
        }
...
const unpauseCommand = async (client, message) => {
    ...
    if (message.isStreamer || message.isMod || message.senderUsername === process.env.DEV_NICK) {
        if (!client.channelConfigs[message.channelName].isPaused) {
            ...
            client.log.logAndReply(
                message,
                `Eu já estou despausado. Se quiser me pausar, use ${message.commandPrefix}pause ${emote}`
            );
            return;
        }
...
export { pauseCommand, unpauseCommand };
