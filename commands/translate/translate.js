...
import { LANGUAGE_MAPPINGS } from './langs.js';
...
const translateCommand = async (client, message) => {
    ...
    if (args.length < 2) {
        client.log.logAndReply(
            message,
            `Use o formato: ${message.commandPrefix}translate <texto para traduzir>`
        );
        return;
    }
    ...
    if (!isLanguageSupported(targetLanguage)) {
        client.log.logAndReply(
            message,
            `O idioma "${targetLanguage}" não é válido. Tente colocar o código do idioma ou em inglês, por exemplo: "pt" ou "portuguese"`
        );
        return;
    }
    ...
    const fromLanguageFullname =
        Object.keys(LANGUAGE_MAPPINGS).find(
            key => LANGUAGE_MAPPINGS[key].toLowerCase() === translateFrom.toLowerCase()
        ) || translateFrom;
    const targetLanguageFullname =
        Object.keys(LANGUAGE_MAPPINGS).find(
            key => LANGUAGE_MAPPINGS[key].toLowerCase() === targetLanguage.toLowerCase()
        ) || targetLanguage;

    client.log.logAndReply(
        message,
        `${capitalize(fromLanguageFullname)} → ${capitalize(targetLanguageFullname)}${translatedText.confidence != 100 ? ` (${translatedText.confidence}%)` : ''}: ${translatedText.translatedText}`
    );
};
...
export { translateCommand, translateText };
