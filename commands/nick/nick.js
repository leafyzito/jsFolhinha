...
async function getAllNicks(userId) {
    const api_url = `https://logs.zonian.dev/namehistory/${userId}`;
...
const nicksCommand = async (client, message) => {
    ...
    client.log.logAndReply(message,
        `${nicksTarget === message.senderUsername ? `O seu histórico de nicks é:`
            : `O histórico de nicks de ${nicksTarget} (id: ${idFromDb}) é:`} ${aliases}`);
};
...
nicksCommand.description = `Exibe o histórico de nicks de um usuário ou de quem executou o comando caso nenhum usuário seja fornecido (desde que o usuário tenha sido visto pelo bot)
• Exemplo: !nicks @leafyzito - O bot irá responder com o histórico de nicks de leafyzito, desde a partir do momento que ele foi visto pelo bot
