...
import { send7tvPresence } from './utils.js';
    ...
    if (message.messageText.startsWith(message.commandPrefix)) {
        ...
        if (command in commandsList) {
            if (
                ...
                (message.channelName == 'whisper' && commandsList[command].whisperable)
            ) {
                commandsList[command](client, message, anonClient).catch(err => {
                    console.log(
...
function listenerHandler(client, message, anonClient) {
    if ([...client.knownUserAliases].length === 0) {
        return console.log('still loading users');
    }

    // client.turso.logMessage(message);

    notifyDevMentionListener(client, message).catch(err => {
