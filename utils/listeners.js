import { timeSince, randomChoice, isStreamOnline } from '../utils/utils.js';
import { afkInfoObjects } from '../commands/afk/afk_info_model.js';
...
const replyMentionListener = async (client, message) => {
    ...
    if (
        ...
        (msgContent[0].toLowerCase() === 'folhinha' ||
            msgContent[0].toLowerCase() === 'folhinhabot' ||
            msgContent[0].toLowerCase() === '@folhinhabot' ||
            msgContent[0].toLowerCase() === '@folhinhabot,')
    ) {
        ...
        client.log.send(
            message.channelName,
            `${message.senderUsername} ${channelEmotes.length > 0 ? randomChoice(channelEmotes) : 'KonCha'}`
        );
    ...
    if (
        msgContent.length === 2 &&
        !message.replyParentMessageID &&
        ['folhinha', 'folhinhabot', '@folhinhabot', '@folhinhabot,'].some(word =>
            msgContent.some(msg => msg.toLowerCase() === word.toLowerCase())
        )
    ) {
        let otherWord = msgContent.find(
            msg => msg.toLowerCase() !== 'folhinha' && msg.toLowerCase() !== 'folhinhabot'
        );

        if (['oi', 'ola', 'olá', 'opa', 'oioi'].some(word => word === otherWord.toLowerCase())) {
            const emote = await client.emotes.getEmoteFromList(
                message.channelName,
                ['peepohey', 'peeposhy', 'eba', 'ola'],
                'KonCha'
            );
...
const afkUserListener = async (client, message) => {
    ...
    if (client.channelConfigs[message.channelName].disabledCommands.includes('afk')) {
        return;
    }
    if (
        client.channelConfigs[message.channelName].offlineOnly &&
    ...
    if (!client.afkUsers[message.channelName].includes(message.senderUsername)) {
        return;
    }
    ...
    client.log.send(
        message.channelName,
        `${message.senderUsername} ${afkReturned} ${afkEmoji} ${afkMessage ? `: ${afkMessage}` : ''} (afk por ${afkSince})`
    );
    ...
    client.afkUsers[message.channelName] = client.afkUsers[message.channelName].filter(
...
const reminderListener = async (client, message) => {
    ...
    if (client.channelConfigs[message.channelName].disabledCommands.includes('remind')) {
        return;
    }
    if (
        client.channelConfigs[message.channelName].offlineOnly &&
    ...
    if (client.notifiedUsers.includes(message.senderUserID)) {
        return;
    }
    ...
    let reminders = await client.db.get(
        'remind',
    ...
    if (reminders.length <= 3) {
        let replyMsg = `${message.senderUsername}, você tem ${reminders.length} lembrete${reminders.length > 1 ? 's' : ''}: `;
        const firstThreeReminders = reminders.slice(0, 3);
        for (const reminder of firstThreeReminders) {
            let remindSender = await client.getUserByUserID(reminder.senderId);
            let remindTimeSince = timeSince(reminder.remindTime);
            replyMsg +=
                ` @${remindSender} (${remindTimeSince}): ${reminder.remindMessage}` +
                (reminder._id === firstThreeReminders[firstThreeReminders.length - 1]._id
                    ? ''
                    : ';');
        }
...
const updateUserListener = async (client, message, anonClient) => {
    if (message.senderUsername === 'folhinhabot') {
        return;
    }

    if ([...client.knownUserAliases].includes(message.senderUsername)) {
        return await updateLastSeen(client, message);
    }

    const knownUsersDB = await client.db.get('users', { userid: message.senderUserID });
    if (knownUsersDB.length > 0) {
        client.discord.log(
            `* Updating user aliases: ${knownUsersDB[0].currAlias} -> ${message.senderUsername}`
        );
        console.log(
            `User found in DB. updating aliases: ${knownUsersDB[0].currAlias} -> ${message.senderUsername}`
        );
        ...
        client.knownUserAliases = client.knownUserAliases.filter(
            alias => alias !== knownUsersDB[0].currAlias
        );
        client.knownUserAliases.push(message.senderUsername);

        if (client.channelConfigs[knownUsersDB[0].currAlias]) {
            client.discord.log(
                `* Updating channel config for ${knownUsersDB[0].currAlias} -> ${message.senderUsername}`
            );
            console.log(
                `Updating channel config for ${knownUsersDB[0].currAlias} -> ${message.senderUsername}`
            );
            await client.db.update(
                'config',
                { channel: knownUsersDB[0].currAlias.toLowerCase() },
            ...
            client.log.send(
                ...
                `Troca de nick detetada: ${knownUsersDB[0].currAlias} -> ${message.senderUsername}`
            );
            ...
            client.channelsToJoin = client.channelsToJoin.filter(
                channel => channel !== knownUsersDB[0].currAlias
            );
...
const notifyDevMentionListener = (client, message) => {
