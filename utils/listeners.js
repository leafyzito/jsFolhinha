...
const replyMentionListener = async (client, message) => {
    ...
    if (msgContent.length === 1 && !message.replyParentMessageID && (msgContent[0].toLowerCase() === 'folhinha' || msgContent[0].toLowerCase() === 'folhinhabot' || msgContent[0].toLowerCase() === '@folhinhabot' || msgContent[0].toLowerCase() === '@folhinhabot,')) {
...
const reminderListener = async (client, message) => {
    ...
    if (reminders.length <= 3) {
        var replyMsg = `${message.senderUsername}, você tem ${reminders.length} lembrete${reminders.length > 1 ? 's' : ''}: `;
        ...
        if (replyMsg.length > 480) {
            ...
            client.log.send(message.channelName, `${message.senderUsername}, você tem ${reminders.length} lembretes. Acesse https://folhinhabot.com/reminders para ver os seus lembretes pendentes (ou use "${message.commandPrefix}remind show all" para ver todos de uma vez)`);
