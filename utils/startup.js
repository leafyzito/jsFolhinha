...
import { MongoUtils } from './mongo.js';
import { Logger } from './log.js';
import { Emotes } from './emotes.js';
import { discordClient } from './discord.js';
import { loadCommands } from '../commands/commandsList.js';
import { timeSince, manageLongResponse, isStreamOnline } from './utils.js';
...
async function modifyClient(client) {
    ...
    client.timeoutUser = async function (message, duration, reason) {
        ...
        const payload = {
            data: { user_id: message.senderUserID, duration: duration, reason: reason },
        };
    ...
    client.whisper = async function (whisperTarget, message) {
        ...
        const response = await fetch(api_url, {
            ...
            console.log(`Error in whisper: ${err}`);
        });
    ...
    client.loadCommands = async function () {
        client.commandsList = await loadCommands();
    };
                    ...
                    try {
                        if (!client.scheduledReminders.includes(reminder._id)) {
                            ...
                            if (reminder.remindAt && reminder.remindAt <= currentTime) {
                                client.discord.log(
                                    `* Sending missed reminder for ${reminderDate.toLocaleString()}`
                                );
                                console.log(
                                    '* Sending missed reminder for ' + reminderDate.toLocaleString()
                                );
                                ...
                                const reminderSender =
                                    (await client.getUserByUserID(reminder.senderId)) ||
                                    'Usuário deletado';
                                const receiverName =
                                    (await client.getUserByUserID(reminder.receiverId)) ||
                                    'Usuário deletado 2';
                                const reminderMessage = timeSince(reminder.remindTime);
                                let finalRes =
                                    reminderSender === receiverName
                                ...
                                const channelName = await client.getUserByUserID(
                                ...
                                console.log(
                                    '* Setting timed reminder for ' + reminderDate.toLocaleString()
                                );
                                ...
                                const job = schedule.scheduleJob(
                                    ...
                                    async function () {
                                        ...
                                        const remindDeletionCheck = await client.db.get(
                                            'remind',
                                            { _id: reminder._id },
                                            true
                                        );
                                        if (
                                            remindDeletionCheck.length != 0 &&
                                            remindDeletionCheck[0].beenRead
                                        ) {
                                            return;
                                        }
                                        ...
                                        if (
                                            !client.channelConfigs[channelName] ||
                                            client.channelConfigs[channelName].isPaused ||
                                            client.channelConfigs[
                                                channelName
                                            ].disabledCommands.includes('remind') ||
                                            (client.channelConfigs[channelName].offlineOnly &&
                                                (await isStreamOnline(channelName)))
                                        ) {
                        ...
                        client.discord.logError(
                            `Error in reminder: ${err} // Reminder: ${reminder}`
                        );
                    }
                }
            }
