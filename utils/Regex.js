...
class CreateRegex {
    ...
    constructor() {
        ...
        this.direct_ping = /\W?ryanpotat\W?/i;
        if (CreateRegex.instance) {
            throw new Error('You may not instantiate singleton more than once, use .new()');
        }
        this.alphabetMap = new Map();
        this.confusablesMap = new Map();
    ...
    get emojis() {
        ...
        this._emojis = new RegExp(/(\u00a9|\u00ae|[\u2000-\u3300]|\ud83c[\ud000-\udfff]|\ud83d[\ud000-\udfff]|\ud83e[\ud000-\udfff])/, 'g');
        return this._emojis;
    }
    get indirect_pings() {
        ...
        this._indirect_pings = new RegExp(/(?<!private\s)(?<![b+:.\-,;`=`])ryan(?:bob|poo?|u|\b(?:n{1,4}))?\b(?![+\-:\-;`=`])(?! (?:renolds|g(?:h)?osl(?:ing|l?ing)\b|higa|garcia|reynolds|lewis|kate|private|magee|\bair(?:lines?|\b)))|(?<![+:.\-,;`=`])\bpotat\b(?![+\-:.\-,;`=`])/, 'g');
        return this._indirect_pings;
    }
    get invisChars() {
        if (this._invisChars) {
        ...
        this._invisChars = new RegExp('[\u034f\u2800\u{E0000}\u000E\u2000\u180e\ufeff\u2000-\u200d\u206D]', 'gu');
        return this._invisChars;
    }
    /** General racism */
    get racism() {
        ...
        this._racism = new RegExp(/((?:(?:\b(?<![-=])|monka)(?:[Nnñ]|[Ii7]V)|[|]\\[|])[\s]*?[liI1y!j|]+[\s]*?(?:[GgbB6934Q🅱qğĜƃ၅5][\s]*?){2,}(?!arcS|l|Ktlw|ylul|ie217|64|\d? ?times))|(\b((?=[nhk])(n[i1!¡jl]b+[e3]r|nygg[e3]r|higger|kneeger)[s5z]?)\b)|((chinam[ae]n|ching[\W_]*chong))|((towel|rag|diaper)[\W_]*head[s5z]?)|((sheep|goat|donkey)\W?(fuck|shag)\w*)|((sand|dune)[\W_]*(n[i1!¡jl]g(?!ht)|c[o0]{2}n|monk[iey]+)\w*)/, 'gimu');
        return this._racism;
    }
    /** Antisemitism */
    get racism2() {
    ...
    get racism3() {
        ...
        this._racism3 = new RegExp(/\b[cĆćĈĉČčĊċÇçḈḉȻȼꞒꞓꟄꞔƇƈɕ]+\b[ÓóÒòŎŏÔôỐốỒồỖỗỔổǑǒÖöȪȫŐőÕõṌṍṎṏȬȭȮȯO͘o͘ȰȱØøǾǿǪǫǬǭŌōṒṓṐṑỎỏȌȍȎȏƠơỚớỜờỠỡỞởỢợỌọỘộO̩o̩Ò̩ò̩Ó̩ó̩ƟɵꝊꝋꝌꝍⱺＯｏ0]{2,}\b[nŃńǸǹŇňÑñṄṅŅņṆṇṊṋṈṉN̈n̈ƝɲŊŋꞐꞑꞤꞥᵰᶇɳȵꬻꬼИиПпＮｎ]+\b[sŚśṤṥŜŝŠšṦṧṠṡŞşṢṣṨṩȘșS̩s̩ꞨꞩⱾȿꟅʂᶊᵴ]*/, 'gimu');
    ...
    get slurs3() {
        ...
        this._slurs3 = new RegExp(/[cĆćĈĉČčĊċÇçḈḉȻȼꞒꞓꟄꞔƇƈɕ]+[hĤĥȞȟḦḧḢḣḨḩḤḥḪḫH̱ẖĦħⱧⱨꞪɦꞕΗНн]+[[iÍíi̇́Ììi̇̀ĬĭÎîǏǐÏïḮḯĨĩi̇̃ĮįĮ́į̇́Į̃į̇̃ĪīĪ̀ī̀ỈỉȈȉI̋i̋ȊȋỊịꞼꞽḬḭƗɨᶖİiIıＩｉ1lĺľļḷḹl̃ḽḻłŀƚꝉⱡɫɬꞎꬷꬸꬹᶅɭȴＬｌ]+[nŃńǸǹŇňÑñṄṅŅņṆṇṊṋṈṉN̈n̈ƝɲŊŋꞐꞑꞤꞥᵰᶇɳȵꬻꬼИиПпＮｎ]+[kḰḱǨǩĶķḲḳḴḵƘƙⱩⱪᶄꝀꝁꝂꝃꝄꝅꞢꞣ]+[sŚśṤṥŜŝŠšṦṧṠṡŞşṢṣṨṩȘșS̩s̩ꞨꞩⱾȿꟅʂᶊᵴ]*/, 'gim');
        return this._slurs3;
    }
    ...
    get tos2() {
        ...
        this._tos2 = new RegExp(/(?=drink)(?<!t )drink (poison|bleach)|(?=slit)(?<!t (have|need) to )slit (y([o0u]+r|o)|ur)|r[a4@]p[e3]\W?(toy|meat|doll|slut|bait|slave|material|[s5$z](l|[^\w\s])([uv]|[^\w\s])[t7]|wh([o0]|[^\w\s])((r|[^\w\s])[e3]|[o0][a@4e3])|hole|face|body|pig)[s5z]?|(?=com+it|end|take)(?<!(n[o\W]t|you) )(?<!\bto )(com+it suicide|(?<!I will )(?<!want to )(?<!wanna )com+it die|(?<!could )(?<!likely )end your( own)? life(?! in\b)|take your( own)? life)|p[e3]d[o0]ph[i1]l[e3]|p[e0]d[o0]|eat (a|my) (dick|cock|penis)|sieg heil|heil hitler/, 'gim');
        return this._tos2;
    }
    /** Sexual harrasment */
    get tos3() {
        ...
        this._tos3 = new RegExp(/pull the [^\s.]+( dam\w*| fuck\S+)? trigger(?! on(?! (yo)?ur\W?self))|blows? (\w+\W+(?<!\.)){1,4}(?<!my )(?<!own )brains? out|(?=play)(?<!to )(?<!n[o\W]t )play in (some )?traffic|(get|be) raped(?! with| on\b)(?<!can be raped)(?<!meant to be raped)|\br[a4@]p([e3][sd]?\b|[i1!¡jl]ng) (her|hi[ms]|the[my]|dem)|throats? (cut|ripped|slit)|pedo|pedobear|(lick|eat|suck|gobble) (my|your|his|her|their|our) (cock|dick|balls|cum|kok|coc)|^get (cancer|a( \w+)? tumor|AIDS|HIV|covid\w*|coronavirus|sick)|\b(suck|lick|gobble|consume|eat)\b.*?\b(my|these)\b.*?\b(cock|penis|dick|balls|nuts)|((show|flash|expose) (you|your|those|them) (tits|boobs|breasts|ass|cock|pussy|vagina|crotch))\b/, 'gim');
        return this._tos3;
    }
    ...
    get tos4() {
        ...
        this._tos4 = new RegExp(/\b(?:(?:i|my age)\s*['’]?\s*(?:am|'m|m| is|will be)\s*(?:(under|below)\s*)?(?:less\s*than\s*)?\s+(1[0-4]|[5-9]|(?:one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve|thirteen)))/, 'gim');
        return this._tos4;
    }
    get tos5() {
        if (this._tos5) {
        ...
        this._tos5 = new RegExp(/(\b(stick|shove|insert|force|put)\b.*?\b(in (my|their|h[eris]{2}|your))\b.*?\b(ass|butt|vagina|asshole|cunt)\b)|(\b([1li][0o][il][il]s?|[1li][0o]l[il]cons?)\b)/, 'gi');
        return this._tos5;
    }
    ...
    get tos6() {
        ...
        this._tos6 = new RegExp(/((k[i1]l+|[e3]nd|sh[0oO]+t)\s?(y[0oO]ur?)\s?(s[e3]l+f)?)/, 'gi');
        return this._tos6;
    }
    /**
     * Sexism
    ...
    get tos7() {
        ...
        this._tos7 = new RegExp(/\bwom[e3]n\s*(belong|should|go)\s*(in|2|go|be|to)?\s*(the\s*)?(k[i1]tch[e3]n|c[0o]{2}k|cl[e3]an)(ing)?\b/, 'gi');
        return this._tos7;
    }
    get gptExtras() {
        if (this._gptExtras) {
    ...
    get url() {
        ...
        this._url = new RegExp(/(http|ftp|https):\/\/([\w_-]+(?:(?:\.[\w_-]+)+))([\w.,@?^=%&:~+#-]*[\w@?^=%&~+#-])/, 'g');
        return this._url;
    }
    ...
    get ableism() {
        ...
        this._abelism = new RegExp(/(?=\br)(?<!\b[a\Wi]m )r+[\W_]*[e3a4@i1!¡jlw]*[\W_]*[t7]+[\W_]*[a4@e3]*[\W_]*r+[\W_]*[dt]+[\W_]*([e3i1!¡jl]+[\W_]*[dt]+[\W_]*)?([s5z]|(?<=retarded)\w+|(?<!retard)ation)?|(th|d|(?=it\W*i?s(?! autism)))((is|at(?! autism)|(?<=th)[ts]|it)([ ''‘’´`]?i?s)?|ese|ose|em) autis(t(ic|ism)?|m)|retard|ass\W?burger/, 'gim');
        return this._abelism;
    }
    get advertising() {
    ...
    get sellerbot() {
        ...
        this._sellerbot = new RegExp(/^(cheap viewers on)|(?:offer|promotion of your) channel|viewers|followers|views|chat bots /, 'gim');
        return this._sellerbot;
    }
    ...
    get twitchEmote1() {
        ...
        this._twitchEmote1 = new RegExp(/^[a-z]{3,10}[A-Z0-9][a-zA-Z0-9]{0,19}$/, 'g');
        return this._twitchEmote1;
    }
    ...
    get twitchEmoteID() {
        ...
        this._twitchEmoteID = new RegExp(/^emotesv2_[a-z0-9]{32}$|^[0-9]{1,12}$/, 'g');
        return this._twitchEmoteID;
    }
    get checkLNPRegex() {
    ...
    get regexSymbolWithCombiningMarks() {
        ...
        this._regexSymbolWithCombiningMarks = new RegExp(/([\0-\u02FF\u0370-\u1AAF\u1B00-\u1DBF\u1E00-\u20CF\u2100-\uD7FF\uE000-\uFE1F\uFE30-\uFFFF]|[\uD800-\uDBFF][\uDC00-\uDFFF]|[\uD800-\uDBFF](?![\uDC00-\uDFFF])|(?:[^\uD800-\uDBFF]|^)[\uDC00-\uDFFF])([\u0300-\u036F\u1AB0-\u1AFF\u1DC0-\u1DFF\u20D0-\u20FF\uFE20-\uFE2F]+)/, 'g');
        return this._regexSymbolWithCombiningMarks;
    }
