class HelixApi {
  ...
  async getManyUsersByUsernames(usernames) {
    if (!Array.isArray(usernames) || usernames.length === 0) {
      return [];
    }

    // Split usernames into chunks of 100 (Twitch API limit)
    const chunkSize = 100;
    const chunks = [];
    ...
    // Process each chunk
    for (const chunk of chunks) {
      const usernamesToUrl = chunk.join("&login=");
      const headers = {
        "Client-ID": process.env.BOT_CLIENT_ID,
        Authorization: `Bearer ${process.env.BOT_OAUTH_TOKEN}`,
      };
      ...
      try {
        const response = await fb.got(
          `${this.baseUrl}/users?login=${usernamesToUrl}`,
          { headers }
        );
